...
def login():
    if request.method == 'POST':
        ...
        if usuario and bcrypt.checkpw(senha.encode('utf-8'), usuario[5].encode('utf-8')):
...
def editar_perfil():
    ...
    if request.method == 'POST':
        ...
        cur.execute("SELECT id FROM usuarios WHERE email = %s AND id != %s", (novo_email, usuario_id))
        usuario_existente = cur.fetchone()
        if usuario_existente:
            cur.close()
